...
const INVITATION_COLLECTION_SCHEMA = Joi.object({
  ...
  // Tytpe of invitation
  /**
   * Tips: Thay vì gọi lần lượt tất cả type của board để cho vào hàm valid() thì có thể viết gọn lại
   * bằng Object.values() kết hợp Spread Operator của JS. Cụ thể: .valid(...Object.values(BOARD_TYPES))
   * Làm như trên thì sau này dù các bạn có thêm hay sửa gì vào cái BOARD_TYPES trong file constants thì
   * ở những chỗ dùng Joi trong Model hay Validation cũng không cần phải đụng vào nữa. Tối ưu gọn gàng
   * luôn.
   */
...
const createNewBoardInvitation = async (data) => {
  try {
    ...
    // Biến đổi một số dữ liệu liên quan tới ObjectId chuẩn chính
    ...
    // Gọi insert vào DB
    const createdInvitation = await GET_DB().collection(INVITATION_COLLECTION_NAME).insertOne(
      newInvitationToAdd
    )
...
// Query aggregate to retrive invitation of specific user
