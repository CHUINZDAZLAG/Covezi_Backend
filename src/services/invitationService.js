...
const createNewBoardInvitation = async (reqBody, inviterId) => {
  try {
    // Người đi mời: chính là người đang request, nên chúng ta lấy theo id lấy từ token
    const inviter = await userModel.findOneById(inviterId)
    // Người được mời: lấy theo email nhận từ phía FE
    const invitee = await userModel.findOneByEmail(reqBody.inviteeEmail)
    // Tìm luôn cả board ra để lấy data xử lý
    ...
    const newInvitationData = {
      ...
      boardInvitation: {
        ...
        status: BOARD_INVITATION_STATUS.PENDING // Default is pending
      }
    }
...
const getInvitations = async (userId) => {
  try {
    ...
    // Convere inviter, invitee, board from array have 1 element to object
...
const updateBoardInvitation = async (userId, invitationId, status) => {
  try {
    ...
    // Creata data to update Invitation
    const updateData = {
      boardInvitation: {
    ...
    // Step 1: Updte status in Invitation
    const updatedInvitaion = await invitationModel.update(invitationId, updateData)

    // Step 2: If accept invitation successfully then add userId to memberIds of collection board
